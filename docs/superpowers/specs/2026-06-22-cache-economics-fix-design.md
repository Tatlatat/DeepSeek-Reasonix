...
**Concurrency note (investigated, not a bug):** the ping is dispatched fire-and-forget; `onTurnStart()` cancels the idle TIMER but does not abort a ping whose `chat()` is already in flight. So an in-flight ping and a real turn can briefly overlap on the same `CacheFirstLoop`. This is safe and cannot corrupt the message log, because both `pingCachePrefix` and `step` build their request via the SYNCHRONOUS `buildMessages()` → `healActiveLogBeforeSend()` → `compactInPlace()` chain, which runs to completion before the first `await` (the `chat()` call). In single-threaded JS, two synchronous blocks cannot interleave, so no two `compactInPlace` calls can overlap; `compactInPlace` (`src/memory/runtime.ts`) is a plain synchronous reassignment, and the heal is idempotent (version-matched fast-path). Once a ping is awaiting `chat()`, it holds only a private copied `messages` array and touches no shared log state. Worst case: a ping sends a slightly stale prefix snapshot — harmless, since the ping writes nothing back to the session. No abort-the-in-flight-ping machinery is needed.
